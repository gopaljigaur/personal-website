...
export function Code({ children, ...props }) {
  ...
  return (
    <div className="relative">
      <button
        onClick={handleCopy}
        className="absolute top-0 right-0 z-10 cursor-pointer rounded p-2 text-neutral-400 transition-colors hover:text-neutral-100 focus-visible:ring-2 focus-visible:ring-neutral-500 focus-visible:outline-none"
      >
        {copied ? (
      ...
      {data.files.map((file, index) => (
        <div key={index} className="group relative overflow-hidden rounded-lg">
          ...
          <div className="flex items-center justify-between border-b border-neutral-200 bg-neutral-100 px-4 py-2 dark:border-neutral-700 dark:bg-neutral-800/50">
            <span className="text-muted-inv text-sm">{file.filename}</span>
            <a
              href={
                data.allFiles
